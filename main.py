...
def button_graspData_click(event):
    ...
    if button_graspData.winfo_containing(event.x_root, event.y_root) == button_graspData:
        ...
        try:
            ...
            # 暂时固定数据包协议和数量用于测试
            packets = sniff(prn=packet_callback, filter="tcp", count=10)
        except Scapy_Exception as e:
            print("抓取数据包出现问题", e)
        else:
...
if __name__ == '__main__':
    ...
    frame_right_top.place(relx=0, rely=0, relwidth=1, relheight=0.75)
    # 创建下半部分的子框架
    frame_right_bottom = tk.Frame(frame_right, bg='yellow')
    frame_right_bottom.place(relx=0, rely=0.75, relwidth=1, relheight=0.25)
    ...
    # 数据库按钮控件
    button_insertToDatabase = tk.Button(frame_left, text='数据库')
